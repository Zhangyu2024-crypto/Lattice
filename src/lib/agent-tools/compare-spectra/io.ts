// I/O boundary for the `compare_spectra` tool: Electron `electronAPI`
...
export function rootApi(): RootFsApi {
  ...
  if (!api) {
    throw new Error('compare_spectra requires the Electron shell.')
  }
...
export async function readAndParse(relPath: string): Promise<ParsedSpectrum> {
  ...
  if (!canParseLocally(relPath)) {
    throw new Error(`Source extension not supported: ${relPath}`)
  }
  ...
  if (!parsed) {
    throw new Error(`Parser returned no spectrum for ${relPath}.`)
  }
