    ...
    async reload_module(env, project_dir, data_packages_js = [], report_applet_versions = false)
    {
        ...
        const Module =
        {
            ...
            callMainWithRedirects(args = [], print = false)
            {
                ...
                // Emscripten's callMain mutates `args` via args.unshift(thisProgram).
                // The bibtex 4-pass path reuses the same pdftex_not_final array
                // across iterations 1 and 3, so the reuse doubles the '/bin/busytex'
                // prefix and the applet dispatcher fails with exit_code 1. Copy
                // defensively so each invocation sees a fresh argv.
                args = args.slice();
