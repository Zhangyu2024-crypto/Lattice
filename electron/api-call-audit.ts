...
export interface ApiCallAuditRuntimeStatus {
  enabled: boolean
  logDir: string
...
export function getApiCallAuditStatus(): ApiCallAuditRuntimeStatus {
  return {
...
export function getApiCallAuditLogDir(): string {
  return config?.dir ?? ''
}

export function clearApiCallAuditLogs(): void {
  if (!config) return
  closeActiveLog()
  queue.length = 0
  queuedBytes = 0
  ...
  for (const name of readdirSync(config.dir)) {
    ...
    rmSync(path.join(config.dir, name), { force: true })
  }
  if (config.enabled) {
    active = openNewLog()
  }
